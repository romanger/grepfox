/* Grepfox Icon System — Lucide-style 2px stroke, square caps, 24px grid */
const ICON_PATHS = {
  ...
  'settings':       'M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6zM19.4 13a1 1 0 0 0 .2 1.1l.1.1a2 2 0 1 1-2.8 2.8l-.1-.1a1 1 0 0 0-1.1-.2 1 1 0 0 0-.6.9V18a2 2 0 1 1-4 0v-.1a1 1 0 0 0-.6-.9 1 1 0 0 0-1.1.2l-.1.1a2 2 0 1 1-2.8-2.8l.1-.1a1 1 0 0 0 .2-1.1 1 1 0 0 0-.9-.6H6a2 2 0 1 1 0-4h.1a1 1 0 0 0 .9-.6 1 1 0 0 0-.2-1.1l-.1-.1a2 2 0 1 1 2.8-2.8l.1.1a1 1 0 0 0 1.1.2H11a1 1 0 0 0 .6-.9V6a2 2 0 1 1 4 0v.1a1 1 0 0 0 .6.9 1 1 0 0 0 1.1-.2l.1-.1a2 2 0 1 1 2.8 2.8l-.1.1a1 1 0 0 0-.2 1.1V11a1 1 0 0 0 .9.6H20a2 2 0 1 1 0 4h-.1a1 1 0 0 0-.9.6z',
  ...
  'github':         'M9 19c-5 1.5-5-2.5-7-3m14 6v-4a3.3 3.3 0 0 0-1-2.6c3-.3 6-1.5 6-6.5a5 5 0 0 0-1.4-3.5 4.7 4.7 0 0 0-.1-3.5s-1.1-.3-3.5 1.3a12 12 0 0 0-6 0C7.6 1.8 6.5 2 6.5 2a4.7 4.7 0 0 0-.1 3.5A5 5 0 0 0 5 9c0 5 3 6.2 6 6.5a3.3 3.3 0 0 0-1 2.5V22',
...
// Logo mark — returns the fox badge as inline HTML.
// Variants: 'badge' (original), 'plate' (void bg + accent fox),
// 'mono-light' (bone fox, transparent), 'mono-accent' (accent fox, transparent),
// 'knockout' (bone fox on accent plate)
function logoMark(size = 40, variant = 'badge') {
  const rx = Math.round(size * 0.175);

  if (variant === 'badge') {
    return `<img src="assets/logo-mark.svg" alt="Grepfox" width="${size}" height="${size}" style="display:inline-block; border-radius:${rx}px; flex-shrink:0">`;
  }

  // Simplified fox art paths for SVG variants
  const foxArt = (earDark, earMid, faceDark, snoutMid, eyeLight, noseDark) => `
    ...
    <path d="M74 95 C 74 178, 98 242, 160 262 C 222 242, 246 178, 246 95 L 246 138 C 222 160, 190 172, 160 172 C 130 172, 98 160, 74 138 Z" fill="${faceDark}"/>
  ...
  const variants = {
    'plate':       { bg: '#0a0a0a', fox: () => foxArt('#ff5a1f','#C4661E','#ff5a1f','#C4661E','#0a0a0a','#0a0a0a') },
    'mono-light':  { bg: 'none',    fox: () => foxArt('#ebe8e3','#ebe8e3','#ebe8e3','#ebe8e3','#0a0a0a','#0a0a0a') },
    'mono-accent': { bg: 'none',    fox: () => foxArt('#ff5a1f','#ff5a1f','#ff5a1f','#ff5a1f','#0a0a0a','#0a0a0a') },
    'knockout':    { bg: '#ff5a1f', fox: () => foxArt('#ebe8e3','#F4F0EA','#ebe8e3','#F4F0EA','#0a0a0a','#0a0a0a') },
  };

  const v = variants[variant] || variants['plate'];
  const bgRect = v.bg !== 'none' ? `<rect width="320" height="320" rx="56" fill="${v.bg}"/>` : '';

  return `<svg width="${size}" height="${size}" viewBox="0 0 320 320" aria-label="Grepfox" style="display:inline-block; flex-shrink:0">${bgRect}${v.fox()}</svg>`;
}

// Logo lockup — badge + GREPFOX text. Gap follows Akrivi method (size * 0.42).
function logoLockup(size = 22, variant = 'badge') {
  const markSize = Math.round(size * 1.6);
  const gap = Math.round(size * 0.42);
  const color = variant === 'knockout' ? '#0a0a0a' : '#ebe8e3';
  return `<div style="display:inline-flex; align-items:center; gap:${gap}px; white-space:nowrap">
    ${logoMark(markSize, variant)}
    <span style="font-family:var(--ff-display); font-weight:700; font-size:${size}px; letter-spacing:-0.04em; line-height:0.88; color:${color}; text-transform:uppercase">Grepfox</span>
  </div>`;
}
...
function icon(name, size = 18, stroke = 2) {
  ...
  if (!d) return '';
  return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${stroke}" stroke-linecap="square" stroke-linejoin="miter" style="flex-shrink:0" aria-hidden="true">${d.split(/(?=M)/).map(sub => `<path d="${sub.trim()}"/>`).join('')}</svg>`;
}
